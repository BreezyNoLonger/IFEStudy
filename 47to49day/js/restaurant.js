class Restaurant{
	...
	// 顾客入座
	seated(menu, customer, restaurant) {
		return new Promise(function(resolve, reject){
			if (restaurant._count < restaurant.seats) {
				restaurant._count++;
				for (let i = 0; i < restaurant.seats; i++) {
					if (restaurant._seat[i] !== '有人') {
						console.log('您的座位号为' + (i+1));
						customer.seatNum = i + 1;
						restaurant._seat[i] = '有人';
						break;
					}
				}
				resolve(menu);
			} else {
				console.log('本店已满课，请排队等候，您前面有' + restaurant._lineNum.length + '个顾客正在等候');
				restaurant._lineNum.push(customer);
			}
		});
